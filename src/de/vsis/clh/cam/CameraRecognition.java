...
import de.vsis.clh.Person;
import de.vsis.clh.PersonDataSource;

/** A basic Camera preview class */
@SuppressLint("NewApi")
public class CameraRecognition extends SurfaceView implements
	...
	private FaceRecognitionOrig faceRec;
	private String personId;
	private Bitmap bmp;
	private Collection<Rect> faceRects;
	private boolean capture = false;
	Person person = new Person();
	private PersonDataSource database = new PersonDataSource(getContext());
	private int[] who;
	...
	public CameraRecognition(Context context, Camera camera, String personId) {
		super(context);
		mCamera = camera;
		...
		faceRec = FaceRecognitionOrig.getInstance();
	}
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		database.open();
		who = new int[database.getAllPersons().size()];
	...
	public void onPreviewFrame(byte[] data, Camera cam) {
		...
		bmp = refreshPicture(data, cam);

		faceRects = findFaces(bmp);

		if (faceRects.size() > 0) {
			if (msg != null)
				msg.cancel();
			for (Rect r : faceRects) {
				getFaceBitmapAndSave(bmp, r);
			}
			faceRec.recognizeFileList(rootPath + "temp.txt");
			int nearest = faceRec.recognizeFileList(rootPath + "temp.txt");
			if (nearest > 0)
				who[nearest - 1]++;
			// Toast.makeText(getContext(), "Recognizing", 1)
			// .show();
			int frame = 0;
			for (int i = 0; i < who.length; i++)
				frame += who[i];
			if (6 < frame) {
				String personID = faceRec.personNames.get(getHighest(who));
				person = database.getPersonByID(personID);
				System.out.println(person.getFirstName());
				String whoString =("(");
				for (int i = 0; i < who.length; i++) {
					whoString = whoString +who[i] + ", ";
				}
				whoString = whoString + ")";
				System.out.println(whoString);
				msg = Toast.makeText(getContext(), person.getFirstName() + "\n" + whoString, 1);
				// // msg.setDuration(10);
				msg.show();
			}
			...
			if (msg != null)
				msg.cancel();
			File f = new File(rootPath + "temp.txt");
			for (int i = 0; i < who.length; i++) {
				who[i] = 0;
			}
			f.delete();
			database.deleteDir(new File(rootPath + "temp/"));
		}

	}

	private int getHighest(int[] who) {
		int highest = 0;
		for (int i = 0; i < who.length; i++) {
			if (who[highest] < who[i])
				highest = i;
		}
		return highest;
	}
	...
	// Log.i(TAG, "Total Recognition process took: " +
	...
	private Bitmap refreshPicture(byte[] data, Camera cam) {
		...
		return Bitmap.createBitmap(mIntArray, width, height,
	...
	private void saveFace(Bitmap bmp, Rect r) {
		File myDir = new File(rootPath + "temp/");
		myDir.mkdirs();
	...
	private void saveImage(Bitmap bmp) {
		...
		try {
			...
			FileOutputStream out = new FileOutputStream(file);
			System.out.println("saved " + fname + " at "
					+ file.getAbsolutePath());
			bmp.compress(Bitmap.CompressFormat.JPEG, 90, out);
			out.flush();
			out.close();

			writer = new FileWriter(rootPath + "temp.txt");
			writer.flush();
			writer.write(n + " unknowen " + file.getAbsolutePath() + "\n");
			writer.close();

		} catch (Exception e) {

			File index = new File(rootPath + "temp.txt");
			try {
				index.createNewFile();
				FileWriter writer2 = new FileWriter(index);
				writer2.flush();
				writer2.write(n + " unknowen " + file.getAbsolutePath() + "\n");
				writer2.close();
	...
	public void getFaceBitmapAndSave(Bitmap bmp, Rect r) {
		// saveImage(, n);
		Bitmap grayFaceBmp = toGrayscale(getFaceBitmap(bmp, r));
		saveImage(grayFaceBmp);
	...
	public Bitmap toGrayscale(Bitmap bmpOriginal) {
		...
		Bitmap resizedGrayBmp = Bitmap.createScaledBitmap(bmpGrayscale, 92,
				112, true);
