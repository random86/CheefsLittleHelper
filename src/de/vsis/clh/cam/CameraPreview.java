...
public class CameraPreview extends SurfaceView implements
	...
	public CameraPreview(Context context, Camera camera, String personId) {
		super(context);
		mCamera = camera;
		...
		camAct = CameraActivity.getInstance();
...
//			faceRec.loadTrainingData();
	...
	public void onPreviewFrame(byte[] data, Camera cam) {
		...
		if (capture) {
			for (Rect r : faceRects) {
				getFaceBitmapAndSave(bmp, r, i);
				// saveFace(bmp, r);
			}

			// Toast.makeText(getContext(), "Photo " + i, Toast.LENGTH_SHORT)
			// .show();
			i++;
			if (i == 15) {
				i = 0;
				Toast.makeText(getContext(), "Photos taken", Toast.LENGTH_SHORT).show();
				capture = false;
				savePreviewImage(bmp);

				// camAct.setResult(Activity.RESULT_OK);
				// Learn
				 train(rootPath+"index.txt");
				// camAct.finish();
			}
		...
		if (faceRects.size() > 0) {
			...
			camAct.capture.setEnabled(true);
			camAct.capture.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					try {
						BufferedReader reader = new BufferedReader(
								new FileReader(rootPath + "index.txt"));

						String lastLine = "";
						while (true) {
							final String line = reader.readLine();
							if (line == null || line.isEmpty()) {
								String[] bla = lastLine.split(" ");
								indexNumber = Integer.valueOf(bla[0]);
								indexNumber++;
								break;
							}
							lastLine=line;
						...
						File index = new File(rootPath + "index.txt");
						try {
							index.createNewFile();
			...
			camAct.capture.setEnabled(false);
		}
	...
	private void saveFace(Bitmap bmp, Rect r) {
		File myDir = new File(rootPath + camAct.ID);
		myDir.mkdirs();
	...
	private void saveImage(Bitmap bmp, int n) {
		FileWriter writer;
		...
		try {
			...
			writer = new FileWriter(rootPath + "index.txt", true);
			writer.flush();
			writer.append(indexNumber+" " + camAct.ID + " " + file.getAbsolutePath()+"\n");
			writer.close();
	...
	private void savePreviewImage(Bitmap bmp) {
		...
		File file = new File(camAct.imageDirection);
		if (file.exists())
			file.delete();
		try {
	...
	public void getFaceBitmapAndSave(Bitmap bmp, Rect r, int n) {
		// saveImage(, n);
		Bitmap grayFaceBmp = toGrayscale(getFaceBitmap(bmp, r));
		saveImage(grayFaceBmp, n);
